...
PopDenDS   := $.File_Composite.PopDenScoreDS;
...
// mincum;
// MortalityScore;
MergeMort := JOIN(MergeEducation,HealthDS,
                    LEFT.State = Right.State,
                    TRANSFORM(CombLayout,
                    ...
                    SELF := LEFT),LOOKUP);
OUTPUT(MergeMort,NAMED('CrimeWeatherEduMort'));
                    
// PopDensityRank
MergeAll := JOIN(MergeMort,PopDenDS,
                    LEFT.State = Right.State,
                    TRANSFORM(CombLayout,
                    SELF.PopDensityRank := RIGHT.PopDensityRank,
                    SELF := LEFT),LOOKUP);
...
CombLayout CompTotal(CombLayout Le) := TRANSFORM
 SELF.ParadiseScore := Le.StudentTeacherScore + Le.PrvSchoolScore + Le.PublicSchoolScore + Le.ViolentScore + Le.PropCrimeScore +
                       Le.MortalityScore + Le.EvtScore + Le.InjScore + Le.FatScore + Le.PopDensityRank;
 SELF := Le;
END;                    

ParadiseSummary := PROJECT(MergeAll,CompTotal(LEFT));

OUTPUT(ParadiseSummary,,'~TeamBelievers::Hack::Output::ParadiseScores',NAMED('Final_Output'),OVERWRITE);
