...
JoinPop := JOIN(SeqRecs,StPop,LEFT.State=RIGHT.state_abbr,
                  TRANSFORM(NewStormRec,
                            SELF.Pop := RIGHT.population,
                            SELF := LEFT)
                 ,LEFT OUTER,LOOKUP);
...
CtTb2 := TABLE(JoinPop,CTRec1,State,State);
OUTPUT(SORT(CtTb2,-StCnt)); 
...
CtTbl := TABLE(JoinState,CTRec,State,Pop);
